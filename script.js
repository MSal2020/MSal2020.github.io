...
// ========== GLOBAL FADE + RISE ANIMATION ==========
// Clean, Apple-like animation for all elements

// Check if coming from page transition (shorter delays) or fresh load (longer delays for loader)
const isFromTransition = sessionStorage.getItem('pageTransitioning') === 'true' || 
                         (!sessionStorage.getItem('hasVisitedSite') === false && 
                          performance.navigation && performance.navigation.type !== 1);
const isReloadOrFirstVisit = (performance.navigation ? performance.navigation.type === 1 : false) || 
                              !sessionStorage.getItem('hasVisitedSite');

// Determine animation delays based on context
const navDelay = isReloadOrFirstVisit ? 1.5 : 0.2;
const starDelay = isReloadOrFirstVisit ? 1 : 0.15;

// Navigation menu items - staggered fade in
...
// Paragraphs - fade + rise (excluding testimonial paragraphs which animate together)
gsap.utils.toArray('p:not(.testimonial-body p)').forEach(p => {
  // Skip if this paragraph is inside testimonial-body
  if (p.closest('.testimonial-body')) return;
...
// Testimonial paragraphs - animate all together
gsap.utils.toArray('.testimonial-body').forEach(testimonialBody => {
  const paragraphs = testimonialBody.querySelectorAll('p');
...
// Lists - fade + rise
gsap.utils.toArray('ul:not(.menu):not(.navigation__list):not(.footer__social)').forEach(ul => {
  gsap.fromTo(ul, {
    opacity: 0,
    y: 20
  }, {
...
// Work experience items - staggered fade + rise
gsap.utils.toArray('.work-rows--item').forEach((item, index) => {
  gsap.fromTo(item, {
    opacity: 0,
    y: 40
  }, {
...
// Category menu - staggered fade + rise
...
function positionCategoryIndicator(categoryLink, animate = true) {
  ...
  if (!indicator || !categoryLink) return;
  ...
  const left = linkRect.left - menuRect.left + menuWrapper.scrollLeft;
...
function filterNotesByCategory(category, skipIndicator = false) {
  ...
  // Use fade + rise animation instead of left/right slide
...
const scrollSpeedThreshold = 170;
const maxPopupOffset = 50;
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  // Migrate old 'darkMode' key to 'theme' for consistency
  if (!currentTheme) {
  ...
  if (currentTheme && toggleSwitch) {
    // Sync both html and body for consistency
    document.documentElement.classList.add(currentTheme);
    document.body.classList.add(currentTheme);
    if (currentTheme === 'dark-mode') {
      toggleSwitch.checked = true;
      if (metaThemeColor) metaThemeColor.setAttribute('content', '#8B4513');
    } else if (metaThemeColor) {
      metaThemeColor.setAttribute('content', '#e7672e');
    }
  }
  ...
  if (toggleSwitch) {
    toggleSwitch.addEventListener('change', function(e) {
      if (e.target.checked) {
        ...
        localStorage.setItem('theme', 'light-mode');
        if (metaThemeColor) metaThemeColor.setAttribute('content', '#e7672e');
      }
    });
  }
  ...
  if (navigation) {
    const navOffsetTop = navigation.offsetTop + 100;
    ...
    // Combined scroll handler with back-to-top
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  if (allCategoryLink) {
    ...
    window.addEventListener('resize', () => {
      ...
      if (activeLink) positionCategoryIndicator(activeLink, false);
    });
    ...
    filterNotesByCategory("all", true);
